...
int main()
{
    ...
    //glScaled((float)SCR_HEIGHT/(float)SCR_WIDTH,1,1); //not support for glfw 3.3+
    ...
    if(SCR_WIDTH>SCR_HEIGHT)
        ...
        glViewport(0, (SCR_HEIGHT-SCR_WIDTH)/2, SCR_WIDTH, SCR_WIDTH);
    */
    //glViewport(a,a,a,a);
    ...
    // point camera at center of the grid, 15 units back from the gridф
...
void framebuffer_size_callback(GLFWwindow* window, int width, int height)
{
    ...
    float Xscale=static_cast<float>(SCR_WIDTH)/static_cast<float>(width);
    float Yscale=static_cast<float>(SCR_HEIGHT)/static_cast<float>(height);
    
    if(Xscale<Yscale)
        glViewport((std::abs(width-width*(Xscale/Yscale)))/2, 0, width*(Xscale/Yscale), height);
    else
        glViewport(0, (std::abs(height-height*(Yscale/Xscale)))/2, width, height*(Yscale/Xscale));
    
    //glViewport(0,0, SCR_WIDTH, SCR_HEIGHT);

    //glViewport((std::abs(width-SCR_WIDTH))/2, (std::abs(height-SCR_HEIGHT))/2, SCR_WIDTH, SCR_HEIGHT);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    float Xscale=static_cast<float>(SCR_WIDTH)/static_cast<float>(w);
    float Yscale=static_cast<float>(SCR_HEIGHT)/static_cast<float>(h);

    if(Xscale<Yscale)
    {
        xpos -= (w-SCR_WIDTH*h/SCR_HEIGHT)/2;
        xpos *= ((SCR_HEIGHT*w)/(SCR_WIDTH*h))*(Yscale);
        ypos *= ((SCR_HEIGHT*w)/(SCR_WIDTH*h))*(Yscale);
    }
    
    else
    {
        ypos -= (h-SCR_HEIGHT*w/SCR_WIDTH)/2;
        ypos *= (SCR_WIDTH*h)/(SCR_HEIGHT*w)*(Xscale);
        xpos *= (SCR_WIDTH*h)/(SCR_HEIGHT*w)*(Xscale);
    }
